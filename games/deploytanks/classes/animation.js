...
function Animation () 
{
	...
	Animation.prototype.recalcPivot = function()
	{
		...
		msglog(this.m_pivotX + ", " + this.m_pivotY);
	}
	...
	Animation.prototype.loadAnimationsFromXml = function(_spriteFile, _animationFile, _targetCollection)
	{
		...
		if (_animations != null)
		{
			...
			for (var iAnim = 0; iAnim < _animations[0].children.length; iAnim++) 
			{
				animationName = _animations[0].children[iAnim].attributes["name"].nodeValue;
				animationLoops = _animations[0].children[iAnim].attributes["loops"].nodeValue;
				...
				pObj.loadAnimationFramesFromXml(_animations[0].children[iAnim], sprites);
				_targetCollection.push(pObj); 
			}
		}
	}
	...
	Animation.prototype.getSpriteSheetName = function(_xmlSprites)
	{
		...
		var spriteSheetName = _spriteSheet[0].attributes["name"].nodeValue;
	...
	{
		animationName = _animationItem.attributes["name"].nodeValue;
		
		for (var i = 0; i < _animationItem.children.length; i++) 
		{
			duration = parseInt(_animationItem.children[i].attributes["delay"].nodeValue);
		
			if (_animationItem.children[i].children.length > 0)
			{
				animData = _animationItem.children[i].children[0].attributes["name"].nodeValue
				animData = animData.replace("/", "");
				offsetX = parseInt(_animationItem.children[i].children[0].attributes["x"].nodeValue);
				offsetY = parseInt(_animationItem.children[i].children[0].attributes["y"].nodeValue);
				...
				if (spriteData != null)
				{
					x1 = parseInt(spriteData.attributes["x"].nodeValue);
					y1 = parseInt(spriteData.attributes["y"].nodeValue);
					x2 = x1 + parseInt(spriteData.attributes["w"].nodeValue);
					y2 = y1 + parseInt(spriteData.attributes["h"].nodeValue);
					incX = 1;
					incY = 1;
