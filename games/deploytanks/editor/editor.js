...
{ 
	...
	{ 
		if (m_keyboard.isKeyDown(C_KEY_SHIFT) == true)
	...
	{
		if (m_keyboard.isKeyDown(C_KEY_RIGHT) == true)
		{
			this.m_ctx = this.m_ctx + 1;
			if (this.m_ctx  > this.m_toolboxCols)
				this.m_ctx = this.m_toolboxCols - 1;

			m_keyboard.disableKeyDownAWhile(C_KEY_RIGHT, 50);
		}
		else if (m_keyboard.isKeyDown(C_KEY_UP) == true)
		{
			this.m_cty = this.m_cty - 1;
			if (this.m_cty  < 0)
				this.m_cty = 0;

			m_keyboard.disableKeyDownAWhile(C_KEY_UP, 50);
		}
		else if (m_keyboard.isKeyDown(C_KEY_LEFT) == true)
		{
			this.m_ctx = this.m_ctx - 1;
			if (this.m_ctx  < 0)
				this.m_ctx = 0;

			m_keyboard.disableKeyDownAWhile(C_KEY_LEFT, 50);
		}
		else if (m_keyboard.isKeyDown(C_KEY_DOWN) == true)
		{
			this.m_cty = this.m_cty + 1;
			if (this.m_cty  > this.m_maze.C_ROWS)
				this.m_cty = this.m_toolboxRows - 1;

			m_keyboard.disableKeyDownAWhile(C_KEY_DOWN, 50);
		}
	...
	EditorMngr.prototype.processEditorCursor = function () 
	{
		...
		else if (m_keyboard.isKeyDown(C_KEY_SPACE) == true)
		{
			m_maze.addObjectToMap(this.m_cx, this.m_cy,  this.getClassID(this.m_selectedResourceId));
			m_keyboard.disableKeyDownAWhile(C_KEY_SPACE, 50);
		}
		else if (m_keyboard.isKeyDown(C_KEY_RETURN) == true)
		{
			this.saveEdition();
			m_keyboard.disableKeyDownAWhile(C_KEY_RETURN, 500);
		}
		else if (m_keyboard.isKeyDown(C_KEY_DELETE) == true)
		{
			this.deleteObjectAtCursorPosition();
			m_keyboard.disableKeyDownAWhile(C_KEY_DELETE, 50);
		}
	...
	EditorMngr.prototype.saveEdition = function()
	{
		...
		if (m_maze.m_lev == null)
			tmpLevel.m_persistenceName = C_EDITOR_NEW_EDITION; 
		else
