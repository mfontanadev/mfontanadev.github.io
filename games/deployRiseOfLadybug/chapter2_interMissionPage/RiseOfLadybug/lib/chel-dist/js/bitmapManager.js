...
{ 
	...
	BitmapManager.prototype.setAlphaCollition = function (_document)
	{
		this.m_document = _document; 
	};
		...
		else
		{
			function updateProgressBar()
			{
				...
				if (arr.length > 0)
				{
	                ...
	                {
	                    chUpdadeInfoControlTextCanvas(_owner.m_idProgressBar, "Loading bitmaps: " + percent + "%");
	                }
				}
			...
			for (var i=0; i < arr.length; i++)
			{
				...
				newItem.m_originalBitmap.onload = function(_event)
				{
					...
					{
						item.m_bitmap = BitmapManager.self.createImageCanvas(BitmapManager.self.m_document, item); 
					}
	...
	BitmapManager.prototype.createImageCanvas = function(_document, _bitmapManagerItem)
	{
		var imgWidth = _bitmapManagerItem.m_originalBitmap.width;
		var imgHeight = _bitmapManagerItem.m_originalBitmap.height;

		var canvasName = "imageCanvas_" + _bitmapManagerItem.m_itemIndex;
