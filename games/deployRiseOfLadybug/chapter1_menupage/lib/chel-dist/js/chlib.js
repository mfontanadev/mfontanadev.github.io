...
function writeMessage(_context, message, _posX, _posY, _size, _fontName, _fontColor, _debug_mode) 
{
	if (_debug_mode == true)
	{
		...
		_context.clearRect(_posX, _posY - _size, textWidth, _size);
		_context.font = font;
...
// ***************************************
function renderCircle(canvas, context, _cenX, _cenY, _radious, _fillColor, _strokeColor, _strokeWidth, _alpha)
{
	context.save();
	context.beginPath();
...
function renderRectangle(canvas, context, _x1, _y1, _w, _h, _fillColor, _strokeColor, _strokeWidth, _alpha)
{
	context.save();
	context.beginPath();
...
function rendercollisionRectangle(canvas, context, _rect, _borderColor)
{
	renderRectangle(
		canvas, 
		context, 
...
function renderLine(canvas, context, _x1, _y1, _x2, _y2, _fillColor, _alpha, _width)
{
	context.save();
	context.beginPath();
...
{ 
	return Math.round( (Math.random() * _max), 0);
}
